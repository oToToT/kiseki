...
void Extend(const int c) {
  ...
  if (!cursor)
    ...
    if ((potential_green->max_len) == (cursor->max_len + 1))
      ...
      Node *wish = new Node((cursor->max_len) + 1);               //從potential_green分離出想要的節點，恰好代表LAST所有的綠⾊字串
      for(;cursor && cursor->edge[c]==potential_green;
           cursor = cursor->green)
        cursor->edge[c] = wish; //添加wish所有的⿊⾊字串，同時可能搶走部分potential_green代表的字串集合
      for (int i = 0; i < 26; i++)
        wish->edge[i] = potential_green->edge[i]; //讓wish接管原本potential_green⿊⾊邊的功能(防⽌potential_green下游的節點代表的字串集合中的⼀些⿊⾊字串，因為potential_green丟掉⼀些⿊⾊字串⽽遺失)
      wish->green = potential_green->green;         //利⽤綠⾊邊添加wish所有的綠⾊字串
      potential_green->green = wish;                //利⽤綠⾊邊修復potential_green代表的字串集合
      LAST->green = wish;                           //利⽤綠⾊邊添加LAST所有的綠⾊字串
    }
  }
}
